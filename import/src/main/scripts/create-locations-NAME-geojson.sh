...
PROJECT_DIR="$1"
GEOM="$PROJECT_DIR/import/src/main/resources/raw-data/PLZO_SHP_LV95/PLZO_OS.shp"
...
ogr2ogr -f GeoJSON -nln NAME \
       -sql "select n.langtext as name, g.geometry as geometry from plzo_os g join plzo_osname n on g.uuid = n.os_uuid order by name" \
       /vsistdout/ $DB_FILE \
